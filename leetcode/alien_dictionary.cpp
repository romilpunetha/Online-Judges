...
string alienOrder(vector<string>& words) {
    ...
    bool x[26][26];
    for(int i = 0; i < 26; i++) for(int j = 0; j < 26; j++) x[i][j] = false;
    
    for(int i = 0; i + 1< words.size(); i++) {

        if(!isValid(words[i], words[i + 1])) return "";
        
        for(int j = 0; words[i][j]; j++) if(indegree.find(get(words[i][j])) == indegree.end()) indegree[get(words[i][j])] = 0;
        for(int j = 0; words[i + 1][j]; j++) if(indegree.find(get(words[i + 1][j])) == indegree.end()) indegree[get(words[i + 1][j])] = 0;
    ...
    string s = "";
    for(auto &it : indegree) if(!it.second) q.push(it.first);
