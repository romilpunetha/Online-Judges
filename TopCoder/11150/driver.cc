...
int main(int argc, char* argv[])
{
	...
	__in >> __expected >> minPrice >> maxPrice;
	if (!__in.good()) break;

	std::cout << "# input for test " << __testnum << ": " << minPrice << ", " << maxPrice << std::endl;
	std::cout.flush();

	__timer_start();

	MicrowaveSelling __object;
	int __actual = __object.mostAttractivePrice(minPrice, maxPrice);
