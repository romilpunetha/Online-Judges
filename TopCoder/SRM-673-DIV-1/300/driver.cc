...
vector <int>	horses;
		...
		__in >> __expected >> warriors >> horses;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << warriors << ", " << horses << std::endl;
		std::cout.flush();

		__timer_start();

		BearCavalry __object;
		int __actual = __object.countAssignments(warriors, horses);
