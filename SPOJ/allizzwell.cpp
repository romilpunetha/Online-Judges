...
int stop = 0;
int xco[] = { 0, 1, 1, 1, 0, -1, -1, -1 };
int yco[] = { -1, -1, 0, 1, 1, 1, 0, -1 };
...
void dfs(vector<string>& str, vvi visited, int a, int b, int r, int c, int index) {
    visited[a][b] = 1;
    FOR(i, 0, 7) {
        // cout<<"i = "<<i<<endl;
        int x = a + xco[i];
        int y = b + yco[i];
        if (isvalid(x, y, r, c) && !visited[x][y] && str[x][y] == st[index]) {
            if (index == 9) {
                stop = 1;
                return;
            }
            dfs(str, visited, x, y, r, c, index + 1);
            if (stop == 1) {
                return;
            }
        }
...
int main() {
    ...
    while (test--) {
        ...
        foreach (it, l) {
            dfs(str, visited, it->ff, it->ss, r, c, 1);
            if (stop) {
                break;
            }
        }
