...
bool util(TreeNode *root,int k){
    ...
    while(1){
        while(l){st1.push(l);l=l->left;}
        while(r){st2.push(r);r=r->right;}
        if(st1.top()==st2.top())return 0;
        if(st1.top()->val+st2.top()->val==k)return 1;
        else if(st1.top()->val+st2.top()->val<k){r=NULL;l=st1.top()->right;st1.pop();}
        else {l=NULL;r=st2.top()->left;st2.pop();}
    }
    return false;
}
