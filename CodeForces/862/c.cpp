...
void solve() {
   ...
   for(int i = 0; i < m; i++) {
        ...
        double x1 = (2.0 * b - sqrt(4 * b * b + 4 * acb));
        double x2 = (2.0 * b + sqrt(4 * b * b + 4 * acb));

        auto it1 = upper_bound(all(lines), 2 * x1);
        if(it1 == lines.end() || (*it1) >= x2) cout << "NO" << endl;
        else cout << "YES\n" << *it1 << endl;
   }
