...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (arr[i - 1] != t)
            mp.insert({ t, i });
        else if (sorted.find(t) == sorted.end())
            sorted[t] = i;
    }
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (mp.find(t) != mp.end()) {
            ...
            res[i] = it->ss;
            sorted[t] = it->ss;
            for (auto &jt : backfill) res[jt] = it->ss;
            backfill.clear();
            mp.erase(it);
        } else if (sorted.find(t) != sorted.end()) {
            res[i] = sorted[t];
            for (auto &jt : backfill) res[jt] = sorted[t];
            backfill.clear();
        } else
            backfill.pb(i);
    }

    if (!mp.empty() || !backfill.empty()) {
        cout << "NO" << endl;
        return;
    }
