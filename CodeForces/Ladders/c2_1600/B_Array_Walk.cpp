...
void solve() {
    ...
    for (int i = 1; i <= m; i++) {
        ...
        for (int j = 1; j <= z; j++) {
            if (i - 2 * j < 0) continue;
            dp[i][j][0] = max(dp[i - 1][j][0], dp[i - 1][j][1]) + arr[i - 2 * j];
            dp[i][j][1] = dp[i - 1][j - 1][0] + arr[i - 2 * j];
        }
    }

    int ans = 0;
    for (int j = 0; j <= z; j++) ans = max(ans, max(dp[m][j][0], dp[m][j][1]));
    cout << ans << endl;
}
