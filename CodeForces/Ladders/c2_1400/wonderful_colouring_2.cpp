...
void solve() {
    ...
    MP<int, Vi> mp;
    for(int i = 0; i < n; i++) {
        int t; cin >> t; mp[t].pb(i);
    }
    Vi arr, res(n, 0);
    for(auto &it : mp) {
        if(it.ss.size() >= k) for(int i = 1; i <= k; i++) res[it.ss[i - 1]] = i;
        else arr.insert(arr.end(), it.ss.begin(), it.ss.end());
    }
    for(int i = 0, m = 1; i < arr.size() - (arr.size() % k); i++) {
        res[arr[i]] = m;
        m = 1 + (m % k);
    }
