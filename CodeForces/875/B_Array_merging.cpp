...
void solve() {
    ...
    int prev1 = arr[0], cnt1 = 1;
    int prev2 = brr[0], cnt2 = 1;
    map<int, int> mp1, mp2;
    for (int i = 1; i <= n; i++) {
        if (arr[i] != arr[i - 1]) {
            mp1[prev1] = max(mp1[prev1], cnt1);
            prev1 = arr[i];
            cnt1 = 1;
        } else
            cnt1++;
    ...
    int ans = 0;
    for (auto &it : mp1) {
        ans = max(ans, it.ss + mp2[it.ff]);
    }

    for (auto &it : mp2) {
        ans = max(ans, it.ss + mp1[it.ff]);
    }
