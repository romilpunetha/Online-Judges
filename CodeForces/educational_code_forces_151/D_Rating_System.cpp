...
pair<int, int> minKadane(Vll arr) {
    int n = arr.size();
    int p = 0, q = 0, r = 0, s = 0;
    ll minEndingHere = LLONG_MAX;
    ll minSoFar = LLONG_MAX;
    for (int i = 0; i < n; i++) {
        if (minEndingHere > 0) {
            minEndingHere = arr[i], p = i, q = i;
        } else {
            minEndingHere += arr[i];
            q = i;
        }
...
void solve() {
    ...
    pair<int, int> t = minKadane(arr);
    ll sum = 0;
    for (int i = t.ff; i <= t.ss; i++) sum += arr[i];
    ...
    sum = 0;
    for (int i = 0; i < t.ff; i++) sum += arr[i];
    cout << sum << endl;
}
