...
bool check(V<Pii> &arr, Vi &queries, int mid, int n) {
    Vi brr(n + 1, 0);
    for (int i = 0; i <= mid; i++) brr[queries[i]] = 1;
    for (int i = 1; i <= n; i++) brr[i] += brr[i - 1];
    for (int i = 0; i < arr.size(); i++) {
        int required = ((arr[i].ss - arr[i].ff + 1) / 2) + 1;
        if (brr[arr[i].ss] - brr[arr[i].ff - 1] >= required) {
            return true;
        }
    }
    return false;
...
void solve() {
    ...
    while (start < end) {
        mid = (start + end) / 2;
        if (check(arr, queries, mid, n))
            end = mid;
        else
            start = mid + 1;
    }
    if (check(arr, queries, start, n))
        cout << start + 1 << endl;
    else
        cout << -1 << endl;
}
