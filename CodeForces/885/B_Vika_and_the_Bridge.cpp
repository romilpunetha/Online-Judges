...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        int t;
        cin >> t;
        if (mp.find(t) == mp.end()) mp[t].push_back(0);
        mp[t].push_back(i);
    }

    for (auto &it : mp) it.ss.push_back(n + 1);

    int res = INT_MAX;

    for (auto &it : mp) {
        vector<int> arr;
        for (int i = 0; i + 1 < it.ss.size(); i++) arr.push_back(it.ss[i + 1] - it.ss[i]);
        int p = arr.size() - 1;
        sort(all(arr));
        int val = (arr[p] + 1) / 2;
        res = min(res, max(val, arr[p - 1]));
    }
    cout << res - 1 << endl;
}
